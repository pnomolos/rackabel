...
### D-104. The hook engine BODY lands; subprocess + timeout/SIGKILL machinery is Unix-only this milestone (DESIGN §5.3/§5.7, milestone 0.5)
...
### D-105. A `pre_deploy` hook whose command cannot be SPAWNED aborts the deploy (it is a refusal, not a skip) (DESIGN §5.3/§5.7)
...
### D-106. on_reload fires per-reloaded-extension with the reload's OVERALL ok/reload_ms (DESIGN §5.3)
