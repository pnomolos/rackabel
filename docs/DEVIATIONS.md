...
### D-102. The hook engine BODY lands (subprocess + EOF-framed stdin + wall-clock timeout); a new `hooks::run` module wraps it for the enumerate verbs (DESIGN §5.3/§5.7, milestone 0.5, HOOK-VERBS)
...
### D-103. The CLI gains `plugin migrate` + a dedicated `PluginEnableArgs`/`PluginMigrateArgs` (DESIGN §2/§5.3/§5.7, milestone 0.5)
...
### D-104. `plugin enable` becomes the hook-consent gate; a declined consent reuses RK0403; a pin change auto-disables with a re-enable prompt (DESIGN §5.7)
