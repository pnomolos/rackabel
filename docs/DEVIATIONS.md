...
### D-56. `dev test` adds the `RK1308 TestFailed` error code (DESIGN §3.8, §7)
...
### D-57. `dev test` gags the 0.2 build's stdout under `--json` instead of adding a `BuildOptions` field
...
### D-58. Foundation `dev test` stub tests updated to the real routing/behavior
